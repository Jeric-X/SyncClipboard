...
本文保留升级基线、分步顺序及验收规则。实际版本与适配结果见[最终配置](DotNet10-And-Dependencies-Upgrade-Summary.md)，各步骤证据见[执行记录](DotNet10-And-Dependencies-Upgrade-Execution.md)，最终提交的 CI 与评审结论见 [PR #419 描述](https://github.com/Jeric-X/SyncClipboard/pull/419)。表中的基线和候选版本不代表当前配置；每一步仍须独立提交、独立验证，通过后才能开始下一步。

**验证范围：所有需要 UI 的内容均不验证。** 保留构建、源码静态检查、不依赖 UI 的自动化测试、协议/数据检查和 CI 打包结果核对。UI 项目仍需升级适配并通过编译，但不启动桌面应用，不执行点击、输入、截图、视觉比较、窗口/焦点/热键/通知实测或依赖交互桌面会话的测试。需要 UI 的检查统一标记为“范围排除：按用户要求不验证”，不作为阶段通过条件，也不因锁屏或缺少可操作桌面而阻塞升级。阶段通过仅代表本文约定的非 UI 验证通过。
...
**2026-09-14 产物验证范围调整：** 按用户最新要求，完全信任 CI 编译和打包产物，不再下载远程编译产物做二进制验证，也不对已下载的远程产物继续做程序集、原生库、签名、包结构、元数据或哈希审计。原先的 55 份产物下载、38 个组合审计和 macOS DMG 下载/挂载不再是阶段或最终交付门槛。确有具体问题需要额外二进制验证时，只验证本地编译的 macOS 版本，不例行重复。非 UI 功能测试、CI 日志/测试报告和评审仍须按步骤独立核对，构建/打包是否成功以当前 head 的 CI 结果为准。
...
| Test SDK、MSTest、coverlet、Moq | 17.14.1 / 3.10.4 / 6.0.4 / 4.20.72 | Test SDK 18.10.0 / MSTest 4.4.0 / coverlet 10.0.1；Moq 4.20.72 已是最新稳定版，保留并验证，保持 VSTest | 16a–16c |
...
同时通过 CI 验证 linux-arm64 和非自包含构建；Windows 上另外验证 Desktop.Default 的 Windows TFM，因为 WinUI3 构建不能覆盖这个入口。
...
| 检查类型 | 本计划处理方式 |
| --- | --- |
| 桌面项目还原、C#/XAML 编译、源码静态分析及 CI 打包结果 | 保留，按当前步骤独立验证 |
| 下载远程编译产物、解包或二进制审计 | 不执行；完全信任对应 CI 的构建与打包成功结果 |
...
| --- | --- | --- |
| 2、3、8、9、18 | 窗口、页面、主题、字体、面包屑、弹窗、托盘的显示与交互；运行时 XAML 加载 | C#/XAML 编译、API 与资源引用静态核对、依赖解析、CI 打包结果，以及隔离的导航逻辑测试 |
| 8、11、12、18 | 真实剪贴板、拖拽、图片预览、热键、通知和桌面元素访问 | 数据转换、图像数据、参数解析、mock 测试及对应非 UI CI 检查 |
...
| 15、17、18 | Swagger UI、IDE 图形操作、安装向导、安装后桌面启动 | HTTP/JSON 检查、命令行构建、服务器冒烟和 CI 打包结果 |
...
2. 独立执行其余构建、非 UI 测试、协议/数据检查并核对 CI 打包结果；任一必要检查失败或缺少证据时，当前步骤保持未通过。
...
| R5 CI 构建与打包 | 核对当前 head 的各平台、架构及包类型构建/打包任务实际成功，完全信任其产物；不下载、不解包、不做远程二进制或哈希审计。确有必要时只检查本地编译的 macOS 版本，仍不启动桌面 UI |

命令日志、测试报告和 CI 记录应标注 OS、架构、配置及对应提交。不采集 UI 截图。范围内测试服务或原生运行环境不可用时记录阻塞，可由对应 CI 承担；仅缺少 UI 会话、设备上的界面操作或 UI 行为证据不构成阻塞。不能把编译、mock 测试或 CI 成功解释为 UI 运行验证。

不再例行审计原生库二进制。只有发现具体问题且确有必要检查本地编译的 macOS 版本时，使用全新发布输出或清理生成缓存；先运行 `dotnet clean src/SyncClipboard.Desktop.MacOS -r osx-arm64 -c Release`，再执行发布和 `build/macos/BundleTool.sh --app-bundle <实际应用包路径> --architecture arm64`，最后检查包内容与严格签名；x64 对应替换架构。只完成 `dotnet publish` 还未覆盖 CI 后续的资源处理和重新签名。
...
**改动：** 暂不升级。确认工作区、分支和远程；记录最新基线 SHA、SDK/MSBuild/Xcode/workload 版本、全部直接依赖及关键传递依赖。保存现有工作流矩阵、基线源码版本及 CI 结果，准备脱敏配置和数据库副本，不下载旧版编译产物。
...
**验证：** 第 3 节原目标框架构建及全部范围内测试；检查实际选中的 SDK，而不只看 setup 安装日志。提交 PR，监控现有及新增范围内检查全部通过，按 R5 核对当前 head 的 CI 构建/打包结果；服务器/容器的功能验证由相应非 UI CI 承担，不下载产物重跑。
...
**验证：** Windows Release 构建和 Test.WinUI3 中的非 UI 用例；现有 x64/arm64、携带/不携带 .NET 和 App SDK 的矩阵；R1、R2、R5。构建与打包以 CI 成功结果为准，不下载或解包，不启动图形安装程序或桌面产物。
...
**通过条件：** Server 和镜像的 CI 构建及非 UI 功能检查通过。默认 .NET 10 镜像基础系统发生变化，需要显式记录选用的镜像标签及基础发行版。[官方容器变更](https://learn.microsoft.com/en-us/dotnet/core/compatibility/containers/10.0/default-images-use-ubuntu)
...
**验证：** 第 3 节所有项目构建、所有范围内测试和 PR 平台矩阵；R1–R5。特别检查各 RID 下 Core 的 `WINDOWS`、`MACOS`、`LINUX` 条件代码。从项目配置、依赖解析和 CI 日志核对运行框架及非自包含部署需求，不下载二进制复验。
...
**验证：** 全平台构建与 Desktop 非 UI 测试、R1/R2/R5；检查 XAML 编译、类型与资源引用、编译绑定、依赖图及 CI 资源打包结果。导航和选择逻辑只通过不初始化 UI 的 ViewModel/集合测试验证，不逐页操作，不运行桌面产物。

**通过条件：** 依赖解析、C#/XAML 编译及范围内测试通过，资源打包 CI 成功；BreadcrumbBar 至少具备源码/API 核对及编译证据，不能只依据 NuGet 还原成功。XAML 运行时加载、视觉效果和交互行为标为“范围排除：按用户要求不验证”，不宣称不存在运行时 UI 问题。[Avalonia 12 迁移说明](https://v11.docs.avaloniaui.net/docs/avalonia12-breaking-changes/)
...
| 9c | H.NotifyIcon.WinUI | 托盘 API/事件签名编译兼容及资源打包 CI；Windows PR 构建及非 UI 测试，不创建真实托盘图标 |
...
**验证：** 图片读取/转换、透明通道、大图、无效图片和缓存；R1/R5；各架构非 UI 图像 CI 与全平台构建成功，不下载程序集或原生库审计。
...
| 子步骤 | 改动 | 独立验证及通过条件 |
| --- | --- | --- |
| 12a | SharpHook | 三平台编译与打包 CI、键盘映射/修饰键纯逻辑和 mock 测试；R1/R2、全平台 PR，不注册真实热键或模拟输入 |
...
| 12e | Microsoft.Toolkit.Uwp.Notifications | 通知载荷、激活参数的纯解析测试及打包 CI；Windows PR；不初始化真实通知服务、不发送通知或执行界面激活 |
...
16c 于 2026-09-14 核定 [Moq 4.20.72](https://www.nuget.org/packages/Moq/4.20.72) 已是最新稳定版，保留该版本及其 Castle.Core 5.1.1 依赖。三套项目在 .NET 10 下解析 Moq 的 net6.0 兼容资产，不要求安装 .NET 6 运行时。独立执行现有 NonUI 测试，保留 Strict、调用次数、参数、属性、异常及回调等断言；Windows mock 和平台服务替身仍由本阶段 PR 验证，不创建真实 UI/COM 服务来替代 mock。
...
| 17c | PupNet 与它需要的工具运行时 | 保留现有包类型和架构，Linux AppImage/deb/rpm 构建与 R5 CI 结果核对通过；若无升级，记录现有工具兼容性证据 |
...
**验证：** 最终 head 上运行第 3 节全部非 UI 验证和 R1–R5；确认范围内 PR 检查、评审、行内线程与普通评论无未解决的可处理问题。从源码、依赖解析和当前 head 的 CI 结果确认版本及约定架构，不下载最终编译产物进行二进制验证。缺少必要非 UI 构建/测试覆盖时由对应 CI 补齐；不要求 UI 会话或界面实测，不缩减构建架构矩阵。

**通过条件：** 记录最终 head、PR、CI runs、测试计数、构建/打包结果和非 UI 验证证据；所有前置步骤已通过，没有“待确认”的目标版本或范围内必要验证。实际 UI 行为统一标注“范围排除：按用户要求不验证”，不作为阻塞，不声称 UI 已验收。删除监控 heartbeat，交由用户决定合并及发布。
...
非 UI 回归编号、结果及证据；CI 构建/打包结论：
远程编译产物处理：完全信任 CI，不下载、不解包、不做二进制审计
额外二进制验证：默认不执行；确有必要时仅限本地编译的 macOS 版本并记录具体原因
