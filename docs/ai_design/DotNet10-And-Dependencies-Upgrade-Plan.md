...
验证范围按用户要求限定为：构建、静态检查、不依赖 UI 的自动化测试、协议/数据检查和打包产物检查。不启动桌面应用进行验收，不执行点击、输入、截图、视觉比较、窗口/焦点/热键/通知实测或依赖交互桌面会话的测试。UI 项目仍需升级适配并通过编译；UI 行为标记为“本计划不验证”，不作为阶段通过条件，也不因锁屏或缺少可操作桌面而阻塞升级。阶段通过仅代表本文约定的非 UI 验证通过。

此范围适用于全部升级步骤、本地验证、PR CI 和后续评审修复。需要 UI 才能完成的检查直接记录为“范围排除”，不安排解锁后的补测，也不以安装虚拟显示器、启动无头 UI 框架或远程桌面代替执行。锁屏时以实际取得的非 UI 命令退出码和 CI 结果为准；若系统休眠导致任务暂停，恢复后继续收集结果，不能提前标记通过。

## 1. 执行规则

1. **严格串行。** 每一步按“确认前置条件 → 锁定版本 → 最小改动 → 本地验证 → 提交并推送 PR → 监控并修复 → 记录通过证据”执行。步骤失败、缺少必要平台验证或等待用户判断时，停止后续升级。
...
**通过条件：** 当前子步骤无生成代码、绑定和集合事件回归；不依靠后一个子步骤修复前一个子步骤的问题。
...
执行时在本文件末尾追加阶段记录或链接到独立执行记录；未执行项保持“待执行”。本地 PR 问题修复日志仍按第 4 节保留，不提交其中内容。
