...
在客户端 `HistoryRecord` 中新增可空字段 `TransferDataFile` 和 `TransferDataHash`，服务端 `HistoryRecordEntity` 新增可空字段 `TransferDataHash`（服务端已经有 `TransferDataFile`）。`TransferDataHash` 的数据库列名为 `TransferDataHash`；历史文件 API 通过 `X-SyncClipboard-Transfer-Data-Hash` 请求/响应头传输，普通剪贴板同步继续使用 `ProfileDto.transferDataHash`。该字段记录“本次实际传输文件的完整字节流”的 SHA-256，统一使用 64 位大写十六进制字符串。
...
本地 `TransferDataFile` 与 `TransferDataHash` 必须在同一次数据库保存中更新。替换 Group ZIP 或其他本地传输文件时，不得出现新路径配旧哈希。删除或判定本地传输文件不可用时同时清空两者；不能为了后续下载而提前保存服务端声明的哈希。
...
- `IsLocalFileReady == false` 时，`TransferDataFile` 和 `TransferDataHash` 都是 `null`；
- 同步合并远端元数据时，永远不从 DTO 更新本地文件路径或哈希；
- 下载文件时从同一个 HTTP 响应取得响应头和文件体，流式校验成功后才把文件路径与哈希一起持久化。

### 历史文件传输协议

`HistoryRecordDto` 不包含 `TransferDataHash` 或 `TransferDataFile`。历史列表、单条元数据查询和 SignalR 变更通知只同步内容语义与记录状态，因此 `SyncRemoteHistoryAsync()` 不会在文件尚未下载时建立任何本地文件哈希绑定。

历史文件上传和下载使用同一个协议头：

```http
X-SyncClipboard-Transfer-Data-Hash: <64 位 SHA-256>
```

- 上传：客户端在 `POST /api/history` 的 multipart 请求头中发送该值，文件仍位于 `data` part；hash 与文件体属于同一次请求；
- 下载：服务端在 `GET /api/history/{profileId}/data` 的响应头中发送该值，文件位于同一次响应体；
- 请求头或响应头存在时必须恰好有一个值，并规范化为大写；格式错误拒绝本次传输；
- 上传声明与服务端实际收到的文件字节不匹配时返回稳定的 422 `history_data_invalid`；即使匹配，新的 Group 记录仍必须完成内部语义校验；
- 下载时客户端先写入唯一临时文件，同时计算 SHA-256；匹配后才原子移动到本地正式路径并建立持久化绑定；
- 旧客户端不发送上传头时，服务端仍按完整校验路径处理并自行计算；旧服务端不返回下载头时，新客户端回退到原有完整 Profile 语义验证。

`TransferDataFile` 仍是客户端或服务端各自的本地内部状态，不通过 API 交换。服务端最终持久化自己计算出的哈希，而不是直接持久化上传请求头的字符串。

原生客户端读取该响应头不受 CORS 限制。若未来允许跨域浏览器 JavaScript 直接调用下载接口，服务端或反向代理还需要暴露 `Access-Control-Expose-Headers: X-SyncClipboard-Transfer-Data-Hash`；常规反向代理应同时保留该自定义请求头和响应头。
...
| 组合 | 行为 |
| --- | --- |
| 新客户端 + 新官方服务端 | 文件请求/响应头与文件体同次传输；整包哈希匹配后可使用服务端已验证绑定 |
| 旧客户端 + 新服务端 | 上传头缺失；服务端完整验证并自行生成；旧客户端忽略下载响应头 |
| 新客户端 + 旧服务端 | 下载响应头缺失；客户端使用旧的完整语义验证路径 |
...
- `Utilities/History/HistoryTransferQueue.cs`：
  - 上传在 `PrepareTransferData()` 后重新取得 Profile 持久化信息，把已验证哈希单独交给文件上传 API；
  - 下载不再预取历史 DTO，直接使用文件响应携带的哈希；
  - 下载流校验成功后才把下载文件路径与哈希随本地记录持久化；
- `RemoteServer/Adapter/OfficialServer/OfficialAdapter.cs`：上传设置请求头；下载读取响应头，流式计算并核对文件 SHA-256，临时文件验证成功后原子发布。

### Server Core

- `Models/HistoryRecordEntity.cs`：持久化已验证字段；`HistoryRecordDto.cs` 明确不携带本地文件绑定；
- `Models/Mapper.cs`：Profile 持久化信息与实体之间传递字段；
- `Controllers/HistoryController.cs`：上传解析并校验请求头，下载在响应头返回服务端已验证哈希；
- `Controllers/SyncClipboardController.cs`：校验普通同步 DTO 的可选字段和暂存传输文件，首次 Group 接收仍执行完整语义校验；
- `Services/History/HistoryService.cs`：上传写盘时计算实际 SHA-256；首次 Group 上传完成语义校验后才保存；更新传输文件时同步更新字段；下载时向 Controller 返回文件路径与已验证哈希。
...
3. 远端记录在本地无数据时不保存服务端文件哈希；本地已有不同 ZIP 时不被远端元数据错误覆盖；
...
2. 旧客户端不发送请求头时上传成功，服务端自行生成并在后续文件下载响应头中返回；
...
6. 历史查询接口和 SignalR 通知不包含 `transferDataHash`，文件下载响应头包含它；
...
- [x] 历史文件 API 通过对称请求/响应头传递 `TransferDataHash`，历史 DTO 不携带该字段；普通剪贴板 `ProfileDto` 仍以可选字段传递。
