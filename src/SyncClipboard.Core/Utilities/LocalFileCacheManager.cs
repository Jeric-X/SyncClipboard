...
{
    public string Id { get; set; } = string.Empty; // 主键（GroupProfile.Hash）
...
public class LocalFileCacheDbContext : DbContext
{
    ...
    {
        _dbPath = Path.Combine(Env.UserAppDataDirectory, "LocalFileCache.db");
    }

    public DbSet<LocalFileCacheEntry> CacheEntries { get; set; }
...
public class LocalFileCacheManager : ILocalFileCacheManager, IDisposable
{
    ...
    public async Task SaveCacheEntryAsync(string cacheType, string id, string filePath, object? metadata = null)
    {
        ...
        try
        {
            ...
            // 检查是否已有相同内容的文件（去重优化）
            var duplicateEntry = await _dbContext.CacheEntries
                .Where(e => e.CacheType == cacheType && e.CachedFileHash == cachedFileHash && e.Id != id)
                .FirstOrDefaultAsync();
                
            if (duplicateEntry != null && File.Exists(duplicateEntry.FilePath))
            {
                // 发现重复内容，使用文件复制代替硬链接
                ...
                File.Copy(duplicateEntry.FilePath, filePath, true);
            }
    ...
    private async Task<bool> IsFileValidAsync(LocalFileCacheEntry entry)
    {
        try
        {
            // 检查文件大小是否匹配
