...
public class HistoryQueryCompatibilityTests
{
    ...
    {
        await using var fixture = await Fixture.CreateAsync(TestContext.CancellationToken);
        HistoryRecordKey[] selected = [new(ProfileType.Text, "text"), new(ProfileType.Image, "image")];

        var records = await fixture.Manager.GetHistoryAsync(filter, token: TestContext.CancellationToken);
        var (totalCount, selectedCount) = await fixture.Manager.GetHistorySelectionCountsAsync(
            filter, null, null, selected, TestContext.CancellationToken);

        Assert.HasCount(expectedCount, records);
    ...
    {
        await using var fixture = await Fixture.CreateAsync(TestContext.CancellationToken);
        var record = await fixture.Db.HistoryRecords.SingleAsync(record => record.Hash == "text", TestContext.CancellationToken);
        record.Stared = true;
        await fixture.Db.SaveChangesAsync(TestContext.CancellationToken);

        var records = await fixture.Manager.GetHistoryAsync(ProfileTypeFilter.Text, started: true,
            before: record.Timestamp.AddSeconds(1), minSize: 1, searchText: "keep", token: TestContext.CancellationToken);
        var counts = await fixture.Manager.GetHistorySelectionCountsAsync(ProfileTypeFilter.Text, true, "keep",
            [HistoryRecordKey.From(record)], TestContext.CancellationToken);
        ...
        Assert.IsEmpty(await fixture.Manager.GetHistoryAsync(ProfileTypeFilter.Text,
            before: record.Timestamp, minSize: 1, token: TestContext.CancellationToken));
    }
    ...
    public async Task SetStarred_UpdatesOnlyExactLiveCompositeKeys()
    {
        ...
        await fixture.Manager.SetStarredAsync([
            new(ProfileType.Text, "text"), new(ProfileType.Image, "image"), new(ProfileType.Text, "deleted")], true, TestContext.CancellationToken);

        var starred = await fixture.Db.HistoryRecords.Where(record => record.Stared).ToListAsync(TestContext.CancellationToken);
        Assert.HasCount(2, starred);
        Assert.Contains(record => record.Type == ProfileType.Text && record.Hash == "text", starred);
        Assert.Contains(record => record.Type == ProfileType.Image && record.Hash == "image", starred);
    }
