...
await QuartzJobChecks.Run(root, Path.GetFullPath(args[1]));

static class QuartzJobChecks
{
    private static readonly JsonSerializerOptions ReportOptions = new() { WriteIndented = true };

    [MethodImpl(MethodImplOptions.NoInlining)]
    public static async Task Run(string root, string reportPath)
    {
        ...
        var dispatch = new UpdateDispatchProbe(updater);
        ...
        collection.AddSingleton(dispatch.Dispatcher.Object);
        ...
        List<string> checks = [];
        var jobs = new JobRunner(services);
        await CheckAppdata(jobs, checks);
        var rows = await CheckHistory(jobs, history, checks);
        await CheckDeletedHistory(jobs, history, rows, checks);
        await CheckOrphanedHistory(jobs, history, rows, checks);
        await CheckLocalCache(jobs, expectedData, checks);
        await CheckUpdate(jobs, config, dispatch, checks);
        await CheckRepeatedCleanup(jobs, checks);
        Check(jobs.CanceledJobs == 6, "all six cancellation entries observed");
    ...
    private static async Task CheckAppdata(JobRunner jobs, List<string> checks)
    {
    ...
    private static async Task<HistoryRecord[]> CheckHistory(JobRunner jobs, HistoryManager history, List<string> checks)
    {
        ...
        await jobs.Canceled<HistoryCleanupJob>();
        ...
        using (var cancellation = new CancellationTokenSource())
        {
            try
            {
                var waitingCleanup = jobs.Execute<HistoryCleanupJob>(cancellation.Token);
        ...
        await jobs.Execute<HistoryCleanupJob>();
    ...
    private static async Task CheckDeletedHistory(JobRunner jobs, HistoryManager history, HistoryRecord[] rows, List<string> checks)
    {
        await jobs.Canceled<DeletedHistoryDataCleanupJob>();
        Check(Directory.Exists(history.GetRecordWorkingDir(rows[5])!), "canceled deleted-data cleanup preserves files");
        await jobs.Execute<DeletedHistoryDataCleanupJob>();
    ...
    private static async Task CheckOrphanedHistory(JobRunner jobs, HistoryManager history, HistoryRecord[] rows, List<string> checks)
    {
    ...
    private static async Task CheckLocalCache(JobRunner jobs, string expectedData, List<string> checks)
    {
    ...
    private static async Task CheckUpdate(JobRunner jobs, ConfigManager config, UpdateDispatchProbe dispatch, List<string> checks)
    {
        await jobs.Canceled<UpdateJob>();
        ...
        dispatch.Fail = false;
        await jobs.Execute<UpdateJob>();
        Check(dispatch.Dispatches == 3, "update recovers after dispatcher failure");
        checks.Add("UpdateJob: dispatcher failure propagation and subsequent recovery");
    }

    private static async Task CheckRepeatedCleanup(JobRunner jobs, List<string> checks)
    {
        await jobs.Execute<AppdataFileDeleteJob>();
        await jobs.Execute<HistoryCleanupJob>();
        await jobs.Execute<DeletedHistoryDataCleanupJob>();
...
sealed class UpdateDispatchProbe
{
